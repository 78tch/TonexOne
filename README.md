...
|No|モード|動作|表示|
|--|--|--|--|
|1|パフォーマンスモード（デュアル・モード）|フットスイッチでプリセットAとBを切り替え。|ALTは緑色に点灯、EQノブがプリセットの色。|
|2|パフォーマンスモード（ストンプ・モード）|フットスイッチでプリセットCとバイパスを切り替え。|ALTがオレンジ色に点灯・消灯、EQノブがプリセットの色。|
|3|グローバルセットアップ|ALTを6秒押し|ALTは青色に点灯、BASSノブがパフォーマンスモードごとの色、MIDがキャビネットバイパスモードの色。|
|4|ブラウジングモード|フットスイッチとALTを同時押し|ALTは紫色に点灯、BASSがプリセットAの、MIDがBの、TREBLEがストンプ（C）のプリセットの色。|
|5|セーフモード|ALTを3秒押しでオン・オフ|マイクロ・ノブが点滅|
|6|チューナー・モード|フットスイッチ長押し（さらにALTでミュート／スルー）|ALTがピンク色点灯|
...
|No|やりたいこと|操作|
|--|--|--|
|1|デュアルモードからストンプモードへ切り替え|ALTを6秒押し、BASSとMIDが点灯・TREBLEが消灯の状態まで押してグローバルセットアップにし、BASSノブを右いっぱいに回すと、緑からオレンジになってストンプモードになり、ALTまたはフットスイッチを押すとグローバルセットアップからパフォーマンスモードに戻る。|
|2|ストンプモードからデュアルモードへ切り替え|ALTを6秒押しグローバルセットアップにし、BASSノブを左いっぱいに回すと、オレンジから緑になってデュアルモードになる。|
|3|チューナーとパフォーマンスモードの切り替え|フットスイッチ長押しすると、ALTが白く点滅してチューナーになる。もう一度フットスイッチ長押しでパフォーマンスモードに戻る。|
|4|チューナー時にミュートとスルーを切り替え|チューナー時にALTを押す|
|5|プリセットAにロードするプリセットを変更する|フットスイッチとALTを同時に押しブラウジングモードにすると、ALTが紫色になる。フットスイッチを押すと、選択対象が移っていくので、BASSを点滅させてから、ノブを回す。左に回すと上（番号が小さい）方に移動、右に回すと下（番号が大きい）方に移動する。もう一度フットスイッチとALTを同時に押すとパフォーマンスモードに戻る。|
...
|8|すべてのプリセットでキャビネットの有無を指定|ALT６秒長押しでグローバルセットアップモードにし、MIDを左に回すとキャビネット有効（ACTIVE）で緑になり、右に回すとキャビネット無効（BYPASS）でオレンジ色になる。|
